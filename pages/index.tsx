...
function Home() {
	...
	const bnbPriceString = bnbPrice ? utils.formatEther(bnbPrice as BigNumber).slice(0, 6) : "0.0";

	const kelpAmount = parseFloat(usdAmount) / parseFloat(kelpPrice);

	const startDateTime = new Date(saleInfo?.startTime ? saleInfo?.startTime.toNumber() * 1000 : 0).toISOString();
	...
	const handleClick = (e: React.MouseEvent<HTMLInputElement>) => {
		...
		if (e.currentTarget.parentElement) {
			e.currentTarget.parentElement.getElementsByTagName("span")[0].style.color = "black";
		}
	...
	const getPartialAmount = (percentage: string) => {
		...
		if (paymentType === "BUSD" && dataBUSD?.value)
			value = getFixedAmount(
				utils.formatEther((dataBUSD?.value as BigNumber).mul(utils.parseEther(percentage)).div(utils.parseEther("1"))),
				2
			);
	...
	const handleBuy = () => {
		...
		if (limitPerAccount !== "0.0" && parseFloat(usdAmount) > parseFloat(limitPerAccount)) {
		...
		<div className="bg-gray-2" style={{ overflowY: "scroll", height: "100vh" }}>
			...
			<main className="container bg-white rounded-xl p-0">
				<div className="md:px-10 md:py-12 xxxs:p-5 container-div" style={{ marginBottom: "50px" }}>
					<section>
						<h1 className="text-gray-1 text-center heading-text">PRIVATE SALE</h1>
					</section>
					...
					{isConnected && <PaymentMethod selectedOption={paymentType} setSelectedOption={setPaymentType} />}
					<div>
						<div className="text-left my-6">
							...
							<div className="flex flex-wrap xxs:justify-between xxxs:justify-center xxxs:mb-2 xxs:mb-0 items-center">
								...
								{isConnected && (
									<div className="flex">
										<p className="me-4 text-gray-1 mb-0 base-options-text" onClick={() => getPartialAmount("0.25")}>
											25%
										</p>
										<p className="me-4 text-gray-1 mb-0 base-options-text" onClick={() => getPartialAmount("0.5")}>
											HALF
										</p>
										<p className="text-gray-1 mb-0 base-options-text" onClick={() => getPartialAmount("1")}>
											ALL
										</p>
									</div>
								)}
